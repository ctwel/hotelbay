...
//		}
		...
		if (type.equals("cart")) {//购物车提交
			...
			for(List<CartVo> checkedGoodsList : newMrchantList) {
				...
				if(couponMerchant != null) {
					...
					if(userCoupon != null) {
						...
						couponId = userCoupon.getId().toString();//优惠券ID
				...
				// 使用的优惠券  身份证
				String backImg = jsonParam.getString("couponId");//留言
				String catchImg = jsonParam.getString("couponPrice");//留言
				orderInfo.setCoupon_id(backImg);
				orderInfo.setCoupon_price(catchImg);
				// 订单金额
			...
			if(StringUtils.isBlank(payType) || "2".equals(payType)) {//普通和秒杀
				...
				orderGoodsVo.setGoods_sn(productInfo.getGoods_id().toString());
