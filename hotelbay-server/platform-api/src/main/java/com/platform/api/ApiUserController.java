...
public class ApiUserController extends ApiBaseAction {
	...
	public Object getFUser(@LoginUser UserVo loginUser) {
		...
		if(mlsUserList!=null &&mlsUserList.size()>0) {
			...
			if(mlsUser!=null) {
				mlsUser.setNickname(Base64Coder.decode(mlsUser.getNickname()));
				mlsUser.setUserName(Base64Coder.decode(mlsUser.getUserName()));
		...
		if(mlsUserList!=null &&mlsUserList.size()>0) {
			...
			if(mlsUserList!=null &&mlsUserList.size()>0) {
				for (MlsUserEntity2 mlsUserEntity2 : mlsUserList) {
					mlsUserEntity2.setNickname(Base64Coder.decode(mlsUserEntity2.getNickname()));
				}
